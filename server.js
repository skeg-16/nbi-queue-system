...
// Google TTS Cache Setup
const ttsCacheDir = path.join(__dirname, 'public', 'tts_cache');
if (!fs.existsSync(ttsCacheDir)) {
    fs.mkdirSync(ttsCacheDir, { recursive: true });
}

// Simple midnight cleanup for cache
setInterval(() => {
    const now = new Date();
    if (now.getHours() === 0 && now.getMinutes() === 0) {
        fs.readdir(ttsCacheDir, (err, files) => {
            if (err) return;
            files.forEach(file => {
                fs.unlink(path.join(ttsCacheDir, file), err => {
                    if (err) console.error("Failed to delete old TTS cache:", err);
                });
            });
        });
    }
...
app.post('/api/announce', async (req, res) => {
    const now = Date.now();
    ttsRateLimit.timestamps = ttsRateLimit.timestamps.filter(ts => now - ts < ttsRateLimit.WINDOW_MS);
    if (ttsRateLimit.timestamps.length >= ttsRateLimit.MAX_REQS) {
        return res.status(429).json({ success: false, error: 'Rate limit exceeded' });
    }
    ttsRateLimit.timestamps.push(now);

    const { number, isPriority, lang } = req.body;
    ...
    const typeStr = isPriority ? 'priority' : 'regular';
    const langStr = lang || 'en';
    const filename = `${dateStr}-${number.replace('#','')}-${typeStr}-${langStr}.mp3`;
    const filepath = path.join(ttsCacheDir, filename);

    // Serve cached file if exists
    if (fs.existsSync(filepath)) {
        return res.json({ success: true, url: `/tts_cache/${filename}` });
    }
    ...
    if (langStr === 'fil') {
        ...
        gender = 'FEMALE';
        if (isPriority) {
            ssml = `<speak><break time="300ms"/><emphasis level="strong">Priority po.</emphasis><break time="500ms"/>Numero <emphasis level="strong">${number}</emphasis> — PWD o Senior Citizen.<break time="400ms"/>Mangyaring pumunta na po sa bintana.</speak>`;
        } else {
            ssml = `<speak><break time="300ms"/>Susunod na po — numero <emphasis level="strong">${number}</emphasis>.<break time="400ms"/>Mangyaring pumunta na po sa bintana.</speak>`;
        }
    } else {
        languageCode = 'en-US';
        if (isPriority) {
            ...
            gender = 'MALE';
            ssml = `<speak><break time="300ms"/><emphasis level="strong">Priority.</emphasis><break time="500ms"/>Now serving number <emphasis level="strong">${number}</emphasis><break time="400ms"/>PWD or Senior Citizen, please proceed to the window.<break time="300ms"/></speak>`;
        } else {
            voiceName = 'en-US-Wavenet-F';
            gender = 'FEMALE';
            ssml = `<speak><break time="300ms"/>Now serving number <emphasis level="strong">${number}</emphasis><break time="400ms"/>Please proceed to the window.<break time="300ms"/></speak>`;
        }
    }
    ...
    const payload = {
        ...
        audioConfig: { audioEncoding: "MP3", speakingRate: 0.90, pitch: 0.0, volumeGainDb: 2.0 }
    };
    ...
    try {
        ...
        res.json({ success: true, url: `/tts_cache/${filename}` });
    } catch (err) {
