...
export default function Dashboard() {
  ...
  const [exportedPdfUrl, setExportedPdfUrl] = useState(null);
  useEffect(() => {
  ...
  async function exportSummary() {
    ...
    // Isang unified table: Section | Metric | Value — madaling buksan/i-sort sa Excel/Sheets
    const rows = [
      { Section: 'Overview', Metric: 'Today', Value: stats.todayTotal },
      { Section: 'Overview', Metric: 'This Week', Value: stats.weekTotal },
      { Section: 'Overview', Metric: 'Priority Today', Value: stats.priorityToday },
      { Section: 'Overview', Metric: 'Waiting Now', Value: stats.waitingNow },
      { Section: 'Priority Breakdown (Last 7 Days)', Metric: 'Priority', Value: priorityBreakdown.priority },
      { Section: 'Priority Breakdown (Last 7 Days)', Metric: 'Regular', Value: priorityBreakdown.regular },
      { Section: 'Priority Breakdown (Last 7 Days)', Metric: 'Priority Rate', Value: `${priorityBreakdown.pct}%` },
      ...(regionData.length === 0
        ? [{ Section: 'Records by Region (Last 7 Days)', Metric: 'No records with region data yet.', Value: '' }]
        : regionData.map(r => ({ Section: 'Records by Region (Last 7 Days)', Metric: r.region, Value: r.count }))
      )
    ];
  ...
  async function exportSummaryPDF() {
    ...
    doc.text('NBI Cybercrime Division - Dashboard Summary', 14, 15);
    doc.setFontSize(9);
    doc.setTextColor(100);
    doc.text(`Generated by: ${genBy}  |  Generated on: ${genOn}`, 14, 21);
    doc.setTextColor(0);
    ...
    autoTable(doc, {
      ...
      startY: 28,
      styles: { fontSize: 9 },
      headStyles: { fillColor: [240, 165, 0], textColor: [30, 30, 30] },
      columnStyles: { 0: { cellWidth: 130 }, 1: { cellWidth: 46 } }
    });

    // Priority breakdown table
    autoTable(doc, {
      head: [['Priority Breakdown (Last 7 Days)', 'Count']],
      body: [
        ['Priority', priorityBreakdown.priority],
        ['Regular', priorityBreakdown.regular],
        ['Priority Rate', `${priorityBreakdown.pct}%`],
      ],
    ...
    // Records by region table
    autoTable(doc, {
      head: [['Records by Region (Last 7 Days)', 'Count']],
      body: regionData.length === 0
        ? [['No records with region data yet.', '']]
    ...
    setExportedPdfUrl(doc.output('bloburl'));
  }
